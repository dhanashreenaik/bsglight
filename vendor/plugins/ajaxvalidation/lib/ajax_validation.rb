module AjaxValidation
  ...
  module Helpers
    def get_validation_url(object)
      "#{url_for(:controller => object.class.to_s.pluralize.underscore, :action => :validate, :id => object.id)}"
		rescue
			"#{url_for(:controller => 'admin/'+object.class.to_s.pluralize.underscore, :action => :validate, :id => object.id)}"
    end
