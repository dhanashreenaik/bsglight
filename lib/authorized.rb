...
module Authorized
  module ModelMethods
    ...
    module InstanceMethods
				...
				def has_container_role(container_id, container_type, role_name)
					return UsersContainer.exists?(:user_id => self.id, :containerable_id => container_id, :containerable_type => container_type.capitalize, :role_id => Role.find_by_name(role_name).id) || self.system_role.name == 'superadmin'
				end
				...
				def container_permissions(container_id, container_type)
					if UsersContainer.exists?(:user_id => self.id, :containerable_id => container_id, :containerable_type => container_type)

						return UsersContainer.find(:first, :conditions => {:user_id => self.id, :containerable_id => container_id, :containerable_type => container_type}).role.permissions
				...
				def has_container_permission(container_type, container_id, permission)
					return !self.container_permissions(container_id, container_type).delete_if{ |e| e.name != permission}.blank? || self.has_system_role('superadmin')
				end
