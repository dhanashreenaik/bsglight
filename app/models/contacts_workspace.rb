  ...
  before_save :create_sha1_id
	...
	def to_group_member(user_id=nil)
		return {
				...
				'email' => begin self.contactable.email_address rescue self.contactable.from_email end,
				...
				'permission' => user_id && ((self.contactable_type=='Person' && self.contactable.user_id==user_id) || User.find(user_id).has_system_role('admin'))
			}
	end
