...
class Admin::ApplicationController < ApplicationController
  ...
  helper :all
	...
	helper_method :available_items_list, :available_languages, :get_sa_config, :right_conf,
		...
		:admin?, :groups_of_workspaces_of_item, :get_fcke_item_types, :setting_searching_params, :get_objects_list_with_search, :current_container_type, :available_profiles
  ...
  # This function will try to execute the 'logged_in?' method (provided by the AutheticatedSystem library)
	...
	def is_logged?
		...
		if session[:latest_register] == true
		   session[:latest_register] = nil	
...
#  end
  ...
  # Checking first the user settings, else taking the default one set inside configuration,
	...
	def get_current_layout
		@search ||= Search.new
    return current_user ? (@current_user.u_layout || @configuration['sa_default_layout']) : 'login'
	end
  ...
  # - workspace : Workspace instance (default: nil)
	def get_allowed_item_types(container=nil)
		if container
			items = (container.available_items.to_s.split(',') & available_items_list)
		else
			items = available_items_list
		end
		return items.sort
	end
  ...
  # - current_workspace : Workspace instance (default: nil)
	def item_types_allowed_to(user, action, current_container=nil)
		if current_container
			items = (current_container.available_items.to_s.split(',') & @configuration['sa_items']).delete_if{ |e| !user.has_container_permission(current_container.class.to_s, current_container.id, e+'_'+action) }
		else
			# TODO something, too huge request, cache ?, and check in authorizable, something special for new, centralize all permission requests
			items = available_items_list.delete_if{ |e| Workspace.allowed_to_user_with_permission(user, e+'_'+action).size == 0 }
		end
		items.sort!
	end
